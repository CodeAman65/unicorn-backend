...
export default function AIResumeArchitect() {
  ...
  const handleGenerate = async () => {
    ...
    setAppState("loading");
    try {
      const response = await fetch("https://unicorn-backend-3.onrender.com/api/generate-resume", {
        method: "POST",
...
try {
  ...
  const handleEditResume = async (e: React.FormEvent) => {
    ...
    setIsEditing(true);
    try {
      const response = await fetch("https://unicorn-backend-3.onrender.com/api/edit-resume", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
